...
%             hold on
        ...
        lmi = y_center(1)-(l0(end)*cosd(beta0(1)));
        r = @(alpha) (sind(alpha)./sind(alpha0))*r0(1);
        l = @(alpha) (cosd(alpha)./cosd(alpha0))*l0(1);
        lmf = y_center(1)-(l(alpha_free)*cosd(beta(alpha_free)));
        % deltalm_free = lmf-lmi;
        options = optimset('display','off');
        [alpha_fzero,~] = fsolve(F_total,alpha0,options);
        deltay = @(alpha) (y_center(1)-(l(alpha)*cosd(beta(alpha))))-lmi;
        ...
        %     deltay = lmf-lmi;
        % else
        %     delta = @(alpha)  lmi-(y_center(1)-(l(alpha).*cosd(beta(alpha))));
...
%         fitness(i,2) = sum(V);
        clear F_total r0 beta0 l0 x_center y_center r l beta t0 alpha_free y F V F_block lmi lmf deltalm_free
    end
