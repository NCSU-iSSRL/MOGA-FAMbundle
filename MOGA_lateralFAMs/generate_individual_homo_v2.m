...
%     x1 = @(alpha) xc_max - ((n_max/2)-1)*deltax_max-(r(alpha)*cosd(beta(alpha)))-(l(alpha)*sind(beta(alpha)));
...
%         x1 = @(alpha) xc_max - ((n_max/2)-1)*deltax_max-(r(alpha)*cosd(beta(alpha)))-(l(alpha)*sind(beta(alpha)));
    ...
    % end
    for i = 1:(n_max/2)
        x1 = xc_max-(i-1)*deltax_max-r0*cosd(beta0);   y1 = yc_max-r0*sind(beta0);
        x2 = xc_max-(i-1)*deltax_max+r0*cosd(beta0);   y2 = yc_max+r0*sind(beta0);
        x3 = x2+l0*sind(beta0);         y3 = y2-l0*cosd(beta0);
        x4 = x1+l0*sind(beta0);         y4 = y1-l0*cosd(beta0);
        % patch([x1 x2 x3 x4],[y1 y2 y3 y4],'r','FaceAlpha',.3)
        % hold on
    ...
    for i = 1:n_max/2
        ...
        individual(i,3) = l0;
        individual(i,4) = xc_max-(i-1)*deltax_max;
        individual(i,5) = yc_max;
    end
